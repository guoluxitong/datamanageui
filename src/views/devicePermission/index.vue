...
<script>
  ...
  import {relation_devicePermissionlistbyconditionandpage,editrelation_devicePermission,deleterelation_devicePermissionbyid} from '@/api/relation_devicePermissionmap'
  export default {
    data() {
      return {
        list: null,
    ...
    methods: {
      ...
      getList() {
        ...
        relation_devicePermissionlistbyconditionandpage(this.listQuery).then(response => {
      ...
      editData(){
        this.$refs.deviceFormData.validate(valid => {
          if (valid) {
            editrelation_devicePermission(this.deviceFormData).then(data=>{
      ...
      handleDelete(row) {
        this.$confirm('确认删除?', '提示', {
          ...
          deleterelation_devicePermissionbyid(row.id).then(data=>{
            this.$message({
              message: '删除成功',
              type: 'success'
            });
