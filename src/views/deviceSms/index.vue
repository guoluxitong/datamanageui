...
<script>
  ...
  import {getRelation_DeviceSmsMapMapperListByConditionAndPage,editRelation_DeviceSmsMap,deleteRelation_DeviceSmsMapById} from '@/api/relation_DeviceSmsMap'
  ...
  export default {
    ...
    methods: {
      ...
      getList() {
        ...
        getRelation_DeviceSmsMapMapperListByConditionAndPage(this.listQuery).then(response => {
      ...
      editData(){
        this.$refs.deviceFormData.validate(valid => {
          if (valid) {
            editRelation_DeviceSmsMap(this.deviceFormData).then(data=>{
      ...
      handleDelete(row) {
        this.$confirm('确认删除?', '提示', {
          ...
          deleteRelation_DeviceSmsMapById(row.id).then(data=>{
            if (this.size == 1) {
